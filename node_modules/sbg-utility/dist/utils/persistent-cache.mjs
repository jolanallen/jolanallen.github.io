...
import { jsonStringifyWithCircularRefs } from './JSON.mjs';
import 'path';
import 'bluebird';
import 'minimatch';
import './filemanager/getAppRootDir.mjs';
import { normalizePath } from './filemanager/path-utility.mjs';
import './logger.mjs';
...
class persistentCache {
  ...
  memoryCache = {};
  constructor(options = {}) {
    this.base = options.base || normalizePath((require.main ? path.dirname(require.main.filename) : undefined) || process.cwd(), 'tmp');
  ...
  unlink(cb) {
    if (this.persist) {
      if (typeof cb !== 'function') {
        return fs.rmSync(this.getCacheDir(), {
          recursive: true,
          force: true
        });
      } else {
        return fs.rm(this.getCacheDir(), {
          recursive: true,
          force: true
        }, safeCb(cb));
      }
