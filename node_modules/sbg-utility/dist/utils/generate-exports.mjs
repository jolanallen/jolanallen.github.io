...
function generateExports({
  ...
  for (const folder of folders) {
    ...
    files.forEach(file => {
      ...
      const entry = {};
      if (fs__default.existsSync(path.join(process.cwd(), `${withoutExt}.d.ts`))) {
      ...
      if (file.endsWith('.mjs')) {
        ...
        entry.require = `./${file}`;
        if (fs__default.existsSync(path.join(process.cwd(), `${withoutExt}.d.cts`))) {
      ...
      if (fs__default.existsSync(path.join(process.cwd(), `${withoutExt}.mjs`))) {
        exportsObj[`./${withoutExt}`].import = `./${withoutExt}.mjs`;
      }
      if (fs__default.existsSync(path.join(process.cwd(), `${withoutExt}.cjs`))) {
  ...
  const pkgOrder = ['name', 'version', 'description', 'keywords', 'homepage', 'bugs', 'license', 'author', 'contributors', 'funding', 'main', 'module', 'types', 'exports', 'files', 'bin', 'directories', 'repository', 'scripts', 'config', 'dependencies', 'devDependencies', 'peerDependencies', 'optionalDependencies', 'engines', 'os', 'cpu', 'private', 'publishConfig'];
  const orderedPkg = {};
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  generateExports({
    pkgPath: path.join(process.cwd(), 'tmp/package.json')
  });
}
