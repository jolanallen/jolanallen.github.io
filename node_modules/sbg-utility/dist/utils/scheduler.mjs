...
async function exitHandler(options, exitCode = 0) {
  ...
  if (options !== null && options !== void 0 && options.cleanup) chain(funcs);
  if (options !== null && options !== void 0 && options.cleanup && scheduler.verbose) _log.info(logname, `clean exit(${exitCode})`);
  if (options !== null && options !== void 0 && options.exit) process.exit();
}
/**
...
function triggerProcess() {
  ...
  if (process.listenerCount('exit') === 0) {
    process.on('exit', exitHandler.bind(undefined, {
      cleanup: true
    }));
  }
  if (process.listenerCount('SIGINT') === 0) {
    process.on('SIGINT', exitHandler.bind(undefined, {
      exit: true
    }));
  }
  if (process.listenerCount('SIGUSR1') === 0) {
    process.on('SIGUSR1', exitHandler.bind(undefined, {
      exit: true
    }));
  }
  if (process.listenerCount('SIGUSR2') === 0) {
    process.on('SIGUSR2', exitHandler.bind(undefined, {
      exit: true
    }));
  }
  if (process.listenerCount('uncaughtException') === 0) {
    process.on('uncaughtException', exitHandler.bind(undefined, {
      exit: true
    }));
  }
}
