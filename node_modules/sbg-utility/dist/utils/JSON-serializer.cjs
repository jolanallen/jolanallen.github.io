...
const __filename$1 = url__namespace.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('utils/JSON-serializer.cjs', document.baseURI).href)));
const {
  parse: $parse,
  stringify: $stringify
} = JSON;
 ...
 */
function getStack() {
  var _Error$stack$split$fi, _Error$stack;
  const stack = (_Error$stack$split$fi = (_Error$stack = new Error('get caller').stack) === null || _Error$stack === void 0 ? void 0 : _Error$stack.split(/\r?\n/gim).filter(str => /(dist|src)/i.test(str) && !str.includes(__filename$1))) !== null && _Error$stack$split$fi !== void 0 ? _Error$stack$split$fi : [];
  const folder = path__namespace.join(process.cwd(), 'tmp/error/json-serializer');
  // Create folder if it does not exist
  if (!fs__namespace.existsSync(folder)) fs__namespace.mkdirSync(folder, {
    recursive: true
  });
...
const stringify = (value, replacer, space) => {
  ...
  const $ = isCallable ? replacer : typeof replacer === 'object' ? (k, v) => k === '' || replacer.indexOf(k) !== -1 ? v : void 0 : noop;
  const known = new Map();
  const input = [];
  const output = [];
