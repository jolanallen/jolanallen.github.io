...
function generateExports({
  ...
  for (const folder of folders) {
    ...
    const files = patterns.flatMap(pattern => glob__namespace.sync(pattern, {
      cwd: folder.dir,
      nodir: true
...
if (process.argv[1] === url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('utils/generate-exports.cjs', document.baseURI).href)))) {
  generateExports({
