...
function capitalize(str, moreSymbols = []) {
  ...
  symbols.forEach(symbol => {
    str = str.split(symbol).map(str => str.charAt(0).toUpperCase() + str.slice(1)).join(symbol);
  });
  return str;
}
...
async function replacePath(source, toReplace, replacement = '') {
  return pathUtility.normalizePath(source).replace(pathUtility.normalizePath(toReplace), replacement);
}
/**
 * slugify string
 * @param str
