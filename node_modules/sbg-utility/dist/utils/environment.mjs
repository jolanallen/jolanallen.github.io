...
function isClass(obj) {
  return typeof obj === 'function' && (/^class\s/.test(Function.prototype.toString.call(obj)) || Object.getOwnPropertyNames(obj.prototype).length > 1);
}
/**
 ...
 */
function get_binary_path(commandName, resolveNodeAbsolutePath = false) {
  if (commandName === 'node') {
  ...
  const cmdPath = [process.cwd(), (process.mainModule || process.main).paths[0].split('node_modules')[0].slice(0, -1)].map(cwd => {
...
export { get_binary_path as getBinaryPath, get_binary_path, isClass, isdev };
