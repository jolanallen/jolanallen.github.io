...
function gulpDebug(filename) {
  var _Error$stack;
  const caller = hash.data_to_hash_sync('md5', ((_Error$stack = new Error('get caller').stack) === null || _Error$stack === void 0 ? void 0 : _Error$stack.split(/\r?\n/gim).filter(str => /(dist|src)/i.test(str))[1]) || '').slice(0, 5);
  ...
  return through2.obj(function (file, _enc, cb) {
    ...
    scheduler.scheduler.add(`${logname} dump ${ansiColors.cyan(caller)} pid ${ansiColors.yellow(String(pid))}`, () => logger.Logger.log(logname, dumpfile));
